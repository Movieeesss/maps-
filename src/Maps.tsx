...
// ─────────────────────────────────────────────────────────────
// Predefined Critical Safety Zones (Structural Issues)
// Edit coordinates to match your actual society layout
// ─────────────────────────────────────────────────────────────
const SAFETY_ZONES: SafetyZone[] = [
  ...
  {
    id: 'sz2',
    label: 'Zone B – Under Repair Slab',
    lat: 10.7895,
    lng: 78.7040,
    radius: 20,
    severity: 'warning',
    description: 'Overhead slab under repair. Wear helmet.',
  },
  {
    id: 'sz3',
    label: 'Zone C – Waterlogging Area',
    lat: 10.7920,
    lng: 78.7030,
    radius: 30,
    severity: 'warning',
    description: 'Poor drainage. Risk of slipping.',
  },
];
...
function createDivIcon(color: string, letter: string) {
  return L.divIcon({
    ...
    html: `<div style="
      ...
      font-weight:900;font-size:14px;color:white;font-family:sans-serif;
    ">${letter}</div>`,
    iconSize: [36, 36],
    iconAnchor: [18, 18],
    popupAnchor: [0, -20],
...
const memberIcon = (name: string, danger: boolean) =>
  createDivIcon(danger ? '#dc2626' : '#16a34a', name[0].toUpperCase());
...
// ─────────────────────────────────────────────────────────────
// Map Controller — smooth follow, only re-centers on significant move
// ─────────────────────────────────────────────────────────────
...
function MemberCard({ member }: { member: MemberLocation }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`
      ...
      shadow-sm mb-2 select-none
    `}>
      <div
        {...attributes}
        {...listeners}
        className="text-slate-300 cursor-grab active:cursor-grabbing px-1"
      ...
      <div className={`
        w-8 h-8 rounded-full flex items-center justify-center text-white font-black text-sm flex-shrink-0
        ${member.inDangerZone ? 'bg-red-500' : 'bg-green-600'}
      `}>
      ...
      {member.inDangerZone && (
        <span className="text-[10px] font-bold text-red-600 bg-red-100 px-2 py-0.5 rounded-full flex-shrink-0 animate-pulse">
          ⚠ ZONE
        </span>
      )}
...
export default function SocietyMaps() {
  // Default center: Trichy — change to your society's GPS
  ...
  useEffect(() => {
    ...
    sock.on('location_update', (data: MemberLocation) => {
      ...
      if (inZone) {
        const z = nearestZone(data.lat, data.lng);
        showAlert(`⚠ ${data.name} entered ${z?.label ?? 'a safety zone'}!`);
      }
    });

    sock.on('sos_alert', (data: { name: string; message: string }) => {
      showAlert(`🆘 SOS from ${data.name}: ${data.message}`);
    });
  ...
  useEffect(() => {
    ...
    const id = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (inZone && !myDanger) {
          showAlert(`⚠ You entered: ${zone?.label}. ${zone?.description}`);
        }
        setMyDanger(inZone);
        setActiveZone(zone);
  ...
  const sendSOS = () => {
    ...
    showAlert('🆘 SOS Alert sent to all society members!');
  };

  const dangerCount = members.filter((m) => m.inDangerZone).length + (myDanger ? 1 : 0);
    ...
    <div className="flex flex-col h-screen w-full overflow-hidden bg-slate-900">
      ...
      <header className="bg-[#0070c0] px-4 py-3 flex items-center gap-3 shadow-lg z-[1000] flex-shrink-0">
        ...
        <div className="flex-1">
          <h1 className="text-white font-black text-base tracking-wide leading-none">SOCIETY LIVE TRACKER</h1>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-white/70 text-[10px]">{isConnected ? 'LIVE' : 'OFFLINE'}</span>
        </div>

        {/* Follow toggle */}
        <button
          onClick={() => setFollow((f) => !f)}
          className={`ml-2 px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-all ${
            follow ? 'bg-white text-[#0070c0] border-white' : 'bg-transparent text-white border-white/40'
          }`}
        >
          {follow ? '⦿ FOLLOW' : '⊙ FREE'}
        </button>
      ...
      {/* ── Danger Zone Warning Bar ───────────────────────── */}
      {myDanger && activeZone && (
        <div className={`flex items-center gap-2 px-4 py-2 flex-shrink-0 ${
          activeZone.severity === 'critical' ? 'bg-red-600' : 'bg-amber-500'
        }`}>
          <span className="text-white text-base">⚠</span>
          <div className="flex-1">
            <p className="text-white font-black text-xs">{activeZone.label}</p>
            <p className="text-white/80 text-[10px]">{activeZone.description}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        >
            ...
            <React.Fragment key={zone.id}>
              <Circle
                ...
                pathOptions={{
                  ...
                  fillColor: zone.severity === 'critical' ? '#fca5a5' : '#fde68a',
                  fillOpacity: 0.35,
                  weight: 2,
                  dashArray: '6 4',
                }}
              >
                <Popup>
                  <div className="font-sans p-1">
                    <p className="font-black text-sm" style={{ color: zone.severity === 'critical' ? '#dc2626' : '#d97706' }}>
                      {zone.label}
                    </p>
                    <p className="text-xs text-gray-600 mt-1">{zone.description}</p>
                    <p className="text-[10px] text-gray-400 mt-1">Radius: {zone.radius}m</p>
                  </div>
                </Popup>
              </Circle>
            </React.Fragment>
          ...
          <Marker position={myPos} icon={myIcon}>
            <Popup>
              <div className="font-sans">
                <p className="font-bold text-[#0070c0]">📍 You are here</p>
                {myDanger && <p className="text-red-600 text-xs font-bold mt-1">⚠ IN SAFETY ZONE</p>}
              </div>
            </Popup>
          </Marker>
          ...
          {members.map((m) => (
            <Marker
              ...
              icon={memberIcon(m.name, m.inDangerZone ?? false)}
            >
              <Popup>
                <div className="font-sans p-1">
                  <p className="font-bold text-sm">{m.name}</p>
                  {m.inDangerZone && <p className="text-red-600 text-xs font-bold">⚠ In Danger Zone!</p>}
        ...
        <button
          onClick={sendSOS}
          className="absolute top-4 left-4 z-[1000] w-14 h-14 rounded-full bg-red-600 text-white font-black text-xs shadow-2xl border-4 border-white flex flex-col items-center justify-center leading-none active:scale-95 transition-transform"
        >
          <span className="text-base">🆘</span>
          <span>SOS</span>
        </button>

        {/* ── Safety Dashboard Toggle ──────────────────── */}
        <button
          onClick={() => setShowPanel((p) => !p)}
          className="absolute top-4 right-14 z-[1000] bg-[#0070c0] text-white text-[10px] font-bold px-3 py-2 rounded-xl shadow-lg"
        >
          {showPanel ? '▼ PANEL' : '▲ PANEL'}
        </button>

        {/* ── Bottom Safety Dashboard ──────────────────── */}
        {showPanel && (
          <div className="absolute bottom-0 left-0 right-0 z-[1000] bg-white/95 backdrop-blur-md rounded-t-3xl shadow-2xl border-t border-slate-200">
            ...
            <div className="grid grid-cols-3 gap-2 px-4 pb-3 pt-1">
              <div className="bg-[#0070c0]/10 rounded-xl p-2.5 text-center">
                <p className="text-[#0070c0] font-black text-xl leading-none">{members.length + 1}</p>
                <p className="text-slate-500 text-[9px] font-bold mt-0.5 uppercase">Tracking</p>
              </div>
              <div className={`rounded-xl p-2.5 text-center ${dangerCount > 0 ? 'bg-red-100' : 'bg-green-100'}`}>
                <p className={`font-black text-xl leading-none ${dangerCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {dangerCount}
                </p>
                <p className={`text-[9px] font-bold mt-0.5 uppercase ${dangerCount > 0 ? 'text-red-400' : 'text-green-400'}`}>
                  In Zone
                </p>
              </div>
              <div className="bg-amber-50 rounded-xl p-2.5 text-center">
                <p className="text-amber-600 font-black text-xl leading-none">{SAFETY_ZONES.length}</p>
                <p className="text-amber-400 text-[9px] font-bold mt-0.5 uppercase">Zones</p>
              </div>
            </div>

            {/* Member list — drag to reorder */}
            <div className="px-4 pb-4 max-h-48 overflow-y-auto">
              <p className="text-[10px] font-black text-slate-400 uppercase mb-2">
                Members — drag ☰ to prioritize
              </p>

              {members.length === 0 ? (
                <p className="text-slate-400 text-xs text-center py-3">
                  No members online yet. Share the app with your family.
                </p>
              ) : (
                <DndContext
                  sensors={sensors}
